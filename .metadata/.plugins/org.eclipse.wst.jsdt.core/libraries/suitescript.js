 ...
 * <ul>
 *     <li>Issue attached to Support Case</li>
 ...
 * </ul>
 * <p>This API is supported in client, user event, scheduled, and Suitelet scripts.
 ...
 * @param {String} type2 [required] - The record internal ID for the record being attached to. Note that if attaching an entity to a static entity group, the internal ID for the entity group record type is <strong>entitygroup</strong>.
 ...
 * @param {Boolean} hideHeader [optional] - If not set, defaults to false. If set to true, the header (navbar/logo) on the assistant is hidden from view.
 ...
 * @returns <strong>Back to</strong> Error Handling APIs | <strong>Back to</strong> SuiteScript Functions
 * @returns See nlobjError - defined in the section on Standard Objects.
 */
 ...
 * <p>This function is available when the Advanced PDF/HTML Templates (Beta) feature is enabled. Because this feature is a beta version, this API is also considered beta and may be subject to change. For information about this feature, see Advanced PDF/HTML Templates (Beta) the help topic Advanced Printing Beta.
 ...
 * @returns issue 199182
 * @returns Additionally an enhancement to nlapiEncrypt() - now includes "standard" sha1 as "ssha1" - at the time QA could not verify this all worked. Code sample below was provided by dev in issue cited above. Verify this sample works before removing the comment tag.
 * @returns var hash = nlapiEncryt('The quick brown fox jumped over the lazy dog');
 * @returns if (hash == 'f6513640f3045e9768b239785625caa6a2588842')
 * @returns nlapiLogExecution('debug','Success!','Matches SHA-1');
 */
 ...
 * @returns strName += nlapiEscapeXML(searchresult.getValue( 'name' ) );
 */
 ...
 * @returns You must use the nlobContext. <strong>getSetting</strong> method on nlapiGetContext to reference script parameters. For example, to obtain the value of a script parameter called custscript_case_field, you must use the following code:
 * @returns {nlobjContext} Context Object
 * @returns Disable SuiteScript in Web services
 * @returns However, both Client and Server SuiteScripts are written to enforce customized business rules that may need to be enforced regardless of the mechanism by which a record is created or updated within NetSuite. This is particularly true for customers who deploy a SuiteCloud partner application and want to be sure their business rules are still respected. Since Client SuiteScript often has browser-specific behavior that requires user action and cannot automatically run during a Web Services call, NetSuite recommends that you disable Client SuiteScript and deploy Server SuiteScript for those business conditions that need to be enforced in all cases.
 * @returns To specify that Server SuiteScript should never execute during a Web services call, enable the <strong>Disable Server-side Scripting</strong> preference on the Web Services Preference page at Setup > Integration > Web Services.
 */
 ...
 * @returns The following sample shows how to use nlapiGetLineItemCount to programatically determine the number of line items on a sublist.
 */
 ...
 * @returns This is a simple client script that throws an alert with the value of the myText variable.
 * @returns <strong>Back to</strong> Sublist APIs | <strong>Back to</strong> SuiteScript Functions
 */
 ...
 * @returns nlapiSendEmail(author, recipient, subject, body, cc, bcc, records, attachments)
 */
 ...
 * <ul>
 *     <li>set the type parameter to 'inventoryitem' to load the follwing types of item records: inventoryitem, lotnumberedinventoryitem, serializedinventoryitem</li>
 ...
 * @returns {Void}
 * @returns This sample creates a new Customer record. When this script runs, execution details are logged on the Execution Log subtab on the Script Deployment page.
 * @returns This snippet shows a search against sales orders, based on specified search filters and search columns. After the search is complete, the remaining units for the script will be logged on the Execution Log tab. If you are worried that your script will exceed unit governance limits, it is useful to track unit usage in the Execution Log.
 * @returns <strong>Back to</strong> Execution Context APIs | <strong>Back to</strong> SuiteScript Functions
 * @returns See nlobjContext - defined in the section on Standard Objects.
 */
 ...
 * <p><strong>Back to</strong> Search APIs | <strong>Back to</strong> SuiteScript Functions
 */
function nlapiLookupField(type, id, fields, text) { };

/**
 * Performs a mail merge operation and returns an nlobjFile object containing the results. This API supports the same records types that are supported in NetSuite's mail merge feature. Supported record types include NetSuite transactions, entities, custom records, and support cases. Note that activity type records such as tasks and events are not currently supported. Also note that transaction column fields are not supported in NetSuite mail merge and therefore are not available through nlapiMergeRecord.
 * <p>This API is supported in user event, scheduled, and Suitelet scripts. It has a governance of 10 usage units.
 * <p>Important: The nlapiMergeRecord function does not support scriptable templates.
 * @param {Number} id [required] - The internalId of the email template
 * @param {String} baseType [required] - The recordType for the primary record used in the merge
 * @param {Number} baseId [required] - The internalId for the primary record used in the merge
 * @param {String} altType [optional] - The recordType for the secondary record used in the merge
 * @param {Number} altId [optional] - The internalId for the secondary record used in the merge
 * @param {Object} fields [optional] - Array of NL tag names to data values (custom name-value merge fields used for merge operation). Note that tag names must begin with NL.
 ...
 * <p>This API was deprecated in NetSuite version 2008.1, however, it continues to be supported. This function will not be enhanced in future versions of NetSuite.
 * <p>In its place, users can use nlapiMergeRecord(id, baseType, baseId, altType, altId, fields), which performs <strong>exactly</strong> the same as nlapiMergeTemplate. The only distinction between the two functions is that nlapiMergeRecord returns a nlobjFile object (instead of a String).
 */
 ...
 * @returns The following example performs a duplicate detection search for all customer records using the “email” field of the currently submitted record.
 */
 ...
 * @returns The following example performs a global search for all records with the keyword <strong>simpson</strong>.
 * @returns In the UI, the results returned from the snippet would look similar to the following:
 * @returns Note that as with global search functionality in the UI, you can programmatically filter the global search results that are returned. In the snippet above, if your first line of code looked like this:
 * @returns var searchresults = nlapiSearchGlobal( ' <strong>cu:</strong> simpson' );
 * @returns only the three Abe Simpson customer records will be returned in your search. For more general information about global search in NetSuite, see Global Search in the NetSuite Help Center.
 */
 ...
 *     <li>To attach emails to custom records. To do this, reference the custom record by either its internalId or scriptId. </li>
 * </ul>
 ...
 * <p>You can send multiple attachments of any media type with this function. Email messages have a 15MB size limit. The total size of the message plus any attachments must be 15MB or less. The size of any individual attachment may not exceed 5MB.
 ...
 * </ul>
 * @param {String} subject [required] - Subject of the outgoing mail.
 * @param {String|nlobjFileObjectreturnedfromnlapiMergeRecord} body [required] - Body of the outgoing email.
 ...
 * @param {Boolean} notifySenderOnBounce [optional] — A value of true causes bounceback notifications to be sent to the original sender for each supplied recipient. If you use this parameter, the maximum number of total recipients (recipient + cc + bcc) allowed is 10. Note that bounceback notification support is dependent upon the recipient's email server settings.
 ...
 */
function nlapiSendEmail(author, recipient, subject, body, cc, bcc, records, attachments, notifySenderOnBounce, internalOnly) { };
 ...
 * <p>Important: This API is not supported within a loop or within a JavaScript array iteration function (for example, map(), some(), filter()). JavaScript loops and array iteration functions are designed to be executed as a whole. SuiteScript cannot yield in the middle of these control structures.
 ...
 * @param {Object} parameters [optional] - An associative array of additional URL parameters. Important: All parameters must be prefixed with <strong>custparam</strong>.
 * @returns {Void}
 ...
 * <p>The nlapiSubmitField(...) function is a companion function to nlapiLookupField(type, id, fields, text).
 ...
 * <p>This API is supported in all script types. See API Governance for the unit cost associated with this API.
 * <p>Important: When using nlapiSubmitRecord in a user event script, it is possible that the related record modified or created by the script is committed to the database but the actual record initiating the script fails on save. To avoid this scenario, SuiteScripts that cause actions on records other than the current one should be set to run <strong>afterSubmit</strong>.
 ...
 * @returns {integer} An integer value of the committed record's internal ID (for example, 555, 21, or 4).
 ...
 * <ul>
 *     <li>transactionType {string} [required] — internal ID of the record type to be voided. See the help topics Supported Transaction Types - Direct Void and Supported Transaction Types — Void by Reversing Journal for a list of valid arguments.</li>
 *     <li>recordId {int} [required] — the internal ID of the specific record to be voided. See the help topic How do I find a record's internal ID? for additional information.</li>
 * </ul>
 * <ul>
 *     <li>An id that indicates the type of void performed</li>
 * </ul>
 * <ul>
 *     <li>CANT_VOID_TRANS — if you attempt to void a transaction that is linked to other transactions (for example, customer payment)</li>
 *     <li>INVALID_RCRD_TYPE — if the transactionType argument passed is not valid</li>
 *     <li>RCRD_DSNT_EXIST — if the recordId argument passed is not valid</li>
 *     <li>THIS_TRANSACTION_HAS_ALREADY_BEEN_VOIDED — if you attempt to void a transaction that has already been voided</li>
 *     <li>VOIDING_REVERSAL_DISALLWD — if you attempt to void a transaction with inventory impact</li>
 * </ul>
 * <ul>
 *     <li>Version 2013 Release 2</li>
 * </ul>
 * <p>The following code creates a new Cash Refund transaction and then voids the transaction. Note that if the Using Reversing Journals preference is enabled, the void will not be successful since Cash Refund is not a supported transaction type for voids by reversing journal. See the help topics Supported Transaction Types - Direct Void and Supported Transaction Types — Void by Reversing Journals.
 * <p><strong>Back to</strong> Record APIs | <strong>Back to</strong> SuiteScript Functions
 */
 ...
 * @returns PDF nlobjFile object
 * @throws Error: ERROR_PARSING_XML (thrown as a user error when XML is badly formed)
 * @since Version 2009.1
 */
 ...
 * <p>Important: This API is not supported within a loop or within a JavaScript array iteration function (for example, map(), some(), filter()). JavaScript loops and array iteration functions are designed to be executed as a whole. SuiteScript cannot yield in the middle of these control structures.
 * @returns Native Javascript Object
 * @returns Be careful if using this API within try / catch / finally. On a successful yield, all the finally blocks will be called, but catches will be ignored.
 * @returns It is advisable to use the finally block for code which is not going to affect program flow, for example - writing log entries.
 * @returns If you have a yield in the try block, it is possible that some instructions in the finally block will execute before the yield takes place. The same instructions will execute again on resume.
 */
 ...
 * @param {Boolean} ordered [required] - A value of <strong>true</strong> means steps must be completed sequentially, and that they will appear vertically in the left panel of the assistant. A value of <strong>false</strong> means steps do not need to be completed sequentially, and they will appear horizontally, directly below the assistant title.
 * ordered parameter set to <strong>true:</strong>
 * ordered parameter set to <strong>false</strong> :
 * @returns {Void}
 * @since Version 2009.2
 * @memberOf nlobjAssistant
 */
 ...
 * <ul>
 *     <li><strong>inline</strong> - This makes the field display as inline text</li>
 *     <li><strong>hidden</strong> - This hides the field on the form.</li>
 *     <li><strong>readonly</strong> - This disables the field but it is still selectable and scrollable (for textarea fields)</li>
 *     <li><strong>entry</strong> - This makes the sublist field appear as a data entry input field (for non checkbox, select fields)</li>
 *     <li><strong>disabled</strong> - This disables the field from user-changes</li>
 *     <li><strong>normal</strong> - (default) This makes the field appear as a normal input field (for non-sublist fields)</li>
 * </ul>
 * @returns {nlobjField} nlobjField
 * @since Version 2008.2
 * @memberOf nlobjField
 ...
 * <ul>
 *     <li><strong>outside</strong> - This makes the field appear outside (above or below based on form default) the normal field layout area</li>
 *     <li><strong>outsidebelow</strong> - This makes the field appear below the normal field layout area</li>
 *     <li><strong>outsideabove</strong> - This makes the field appear above the normal field layout area</li>
 *     <li><strong>startrow</strong> - This makes the field appear first in a horizontally aligned field group in normal field layout flow</li>
 *     <li><strong>midrow</strong> - This makes the field appear in the middle of a horizonatally aligned field group in normal field layout flow</li>
 *     <li><strong>endrow</strong> - This makes the field appear last in a horizonatally aligned field group in normal field layout flow</li>
 ...
 * <ul>
 *     <li><strong>startcol</strong> - This starts a new column (also disables automatic field balancing if set for any field)</li>
 *     <li><strong>startrow</strong> - For outside fields, this places the field on a new row</li>
 ...
 * <ul>
 *     <li><strong>breadcrumb</strong> - appears on top left corner after system bread crumbs</li>
 *     <li><strong>crosslink</strong> - appears on top right corner</li>
 * </ul>
 ...
 * @param {String} name [required] - The internal ID name of the sublist. The internal ID must be in lowercase, contain no spaces, and include the prefix custpage if you are adding the sublist to an existing page. For example, if you add a sublist that appears on the UI as Purchase Details, the sublist internal ID should be something equivalent to <strong>custpage</strong>_purchasedetails or <strong>custpage</strong>_purchase_details.
 * @param {enumString} type [required] - The sublist type. Use any of the following types:
 * <ul>
 *     <li><strong>editor</strong> - An edit sublist with non-inline form fields (similar to the Address sublist)</li>
 *     <li><strong>inlineeditor</strong> - An edit sublist with inline fields (similar to the Item sublist)</li>
 *     <li><strong>list</strong> - A list sublist with editable fields (similar to the Billable Items sublist)</li>
 *     <li><strong>staticlist</strong> - A read-only segmentable list sublist (similar to the search results sublist)</li>
 * </ul>
 ...
 * @returns If there are multiple values selected in the multi-select field, this method returns a string array of field IDs.
 * @returns If the field is not on the record, this method returns null.
 * @memberOf nlobjRecord
 */
 ...
 * @param {String} name - The internal ID of the search field. For example, if one of your filtering criterion is Quantity Available, you will set the value of name to <strong>quantityavailable</strong>, which is the search field ID for Quantity Available.
 ...
 * @param {String|Date} value2 - A secondary filter value -or- special date field value for between/within style operators * lastbusinessweek. See the following list of possible values:
 * @returns {nlobjSearchFilter} nlobjSearchFilter
 * @since Version 2007.0
 * @memberOf nlobjSearchFilter
 */
nlobjSearchFilter.prototype.constructor = function(name, join, operator, value1, value2) { };
...
/**
 * Can be used on formula fields and non-formula (standard) fields to get the value of a specified column
 * @param {nlobjColumn} column [required] - Column object used in search whose value you want to return
 * @returns {String} String value of the column
 * @since Version 2009.1
 * @memberOf nlobjSearchResult
 */
 ...
 *     <li>select</li>
 *     <li>url</li>
 *     <li>image - Important: This field type is available <strong>only</strong> for fields appearing on list/staticlist sublists. You cannot specify an image field on a form.</li>
 *     <li>timeofday</li>
 *     <li>textarea</li>
 *     <li>percent</li>
 ...
 * @param {String} name [required] - variable name that represents record
 * @param {nlobjRecord} record [required] - NetSuite record
 * @returns {Void}
 * @since Version 2013.1
 * @memberOf nlobjTemplateRenderer
 */
nlobjTemplateRenderer.prototype.addRecord = function(name, record) { };

/**
 * Binds nlobjSearchResult object to variable name used in template.
 * @param {String} name [required] - variable name that represents search result
 * @param {nlobjSearchResult} searchResult [required] - NetSuite search result
 * @returns {Void}
 * @since Version 2013.1
 * @memberOf nlobjTemplateRenderer
 */
nlobjTemplateRenderer.prototype.addSearchResults = function(name, searchResult) { };
